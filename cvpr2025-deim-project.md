...
1. 本项目支持简单学习和深入学习的方式，我们为不同需求的人群针对性制作了两版教程，其中一版是针对一些不想深入学习代码的同学(偏向傻瓜式操作)，使其可以在1～2天内可以上手改进实验，基本不涉及改代码，都是改改配置文件就可以完成模型上的改进。另外一版是针对一些想深入学习代码的同学，这类同学愿意花时间去深入学习，视频/文档教程非常充足。
2. 这个项目会**不定时(直播时间到时候会群里进行通知，没有硬性规定多久一次，不方便看的会有录播)**有**直播**，详细直播内容请看第三大点。
3. 这个项目会持续更新创新点，如果创新点是来源于现有的模型，还会提供对应的论文来源和模块说明（假设像FasterNet中的FasterBlock，会提供好对应的py文件），用户可以根据从本课程学习到的缝合模块（代指第一点）去定制或者创新自己的网络。
...
PS:
1. 20250614版本更新后，本项目的dfine和cvpr2025-deimv1已经支持Ultralytics同款的配置文件形式，大大降低上手难度！[B站介绍链接](https://www.bilibili.com/video/BV1Q4MHzXEdd/)
2. 20260530版本更新后，本项目有一个Easy版本的UserGuide-Easy.md，适合部分不想花太多时间去学代码的同学，1～2天内就可以上手跑改进实验。
...
1. 本项目价格为348，没有时效限制。
2. 虚拟项目一经售出不退不换，需要入手前考虑清楚，如果你是初次入手我的项目，怕我不靠谱，可以先考虑入手个YOLO或者RTDETR看下。
...
- 20260515
    ...
    1. 新增图像-文本多模态检测（Semantic-Aware Closed-Vocab Detection）。目前支持CVPR2024-MobileClip、TMLR2025-MobileClip2、ICLR2025-TIPS、CVPR2026-TIPSv2作为文本编码器，支持对文本特征做特征适配，支持把文本特征融合到fpn内做信息交互。新增的图文多模态可以使用Ultralytics-Yaml来进行搭建，大大降低使用难度。
    2. 支持 图像-图像(示例是深度图，但不限制于深度图，只要是单通道的数据，都可以转换成适用的格式)-文本 三模态检测。
    3. 对 show_yml_param.py 进行重构，使其输出参数的时候更易懂，脚本已经移植到 tools/utils 内。
    4. 新增 tools/utils/compare_yml.py，用于对比两个yml的参数差异。
    5. 对configs里面的内容进行整理，把关于Ultralytics配置文件部分的cfg、cfg-improve、yaml部分进行合并到configs/ultralytics-yaml内。
    6. 对UserGuide.md进行分拆，把更新日志和模块列表单独出来一个文件UpdateLog.md,ModuleList.md。
    7. 增加训练开始的时候学习率的输出细节。
    8. 增加图图多模态推理脚本 tools/inference/mutilmodality/torch_inf.py。
    9. 优化图图多模态的归一化部分，如果另外一个模态的值已经是0-1，则跳过归一化部分，如果大于1，则会按0-255进行归一化。
    10. 优化图图多模态的npy、npz格式读取。
    11. pycocoeval增加了verytiny指标，如需使用需要重新安装pycocoeval，并且看UserGuide.md中的额外参数说明<using_vtiny_metrice>。
    12. 新增TGRS2026-CKConv模块。
    13. 新增AAAI2026-RFGM模块。
    14. 新增AAAI2026-PatchMamba模块。
    15. 新AAAI2026-GradMamba模块。
...
- engine/extre_module/custom_nn/mamba
    ...
    15. AAAI2026|engine/extre_module/custom_nn/mamba/GradMamba.py
    16. AAAI2026|engine/extre_module/custom_nn/mamba/PatchMamba.py
...
3. featuremap脚本使用教程
4. heatmap脚本使用教程
5. tools/inference/torch_inf.py教程
6. 20250513基础疑问解答直播回放
7. 20250618基础疑问解答直播回放
8. 20250704基础疑问解答直播回放
9. 20250910基础疑问解答直播回放
10. DEIMV2移植代码说明
...
1. 小目标检测网络架构专题一群课题直播回放
2. 小目标检测网络架构专题二
3. 顶会中的Partial创新思想课程
4. CVPR2025-MobileMamba中的Long-Range WTB-Mamba二次创新直播回放
5. 自监督最强巨兽DINOV3在DEIM、DFine中的配置文件说明和注意点
6. DQ-DETR的创新系列一｜如何在自己的yml上使用DQ-DETR的思想
7. DQ-DETR的创新系列二｜如何拆解Dynamic-Query和Counting-Guided Feature Enhancement
8. DQ-DETR的创新系列三｜DQDFineTransformer计算量太大了？怎么去分析？怎么去降？怎么去创新？
9. DQ-DETR的创新系列四｜CGFE是什么？怎么去创新？
10. DQS-DETR的创新系列一｜如何在自己的yml上使用DQS
11. DQS-DETR的创新系列二｜DQS流程讲解
12. TGRS2025-HighFrequencyDirectionInjection创新思想课程
13. ES-MoE动态路由网络教程
14. YOLO-MS中的MSBlock与GQL教程
